...
# def train_model_bundle(ex, symbol: str, timeframe: str, lookback: str):
#     """Fetch data, build features, train a small classifier. Return bundle or None."""
#     try:
#         df_train = fetch_ohlcv_lookback(ex, symbol, timeframe=timeframe, lookback=lookback)
#         if len(df_train) < 400:
#             return None
...
#         except Exception:
#             base = df_train["close"].astype(float)
#         ret = (base.shift(-H) / base - 1.0)
#         y = (ret > 0).astype(int)
#         X = Xf_train.copy()
#         y = y.iloc[:-H]; X = X.iloc[:-H]
...
#         clf.fit(X[cols], y)
#         bundle = {"model": clf, "cols": cols, "tf": timeframe, "horizon": H}
#         return bundle
#     except Exception:
#         return None
...
def fetch_ohlcv_lookback_safe(ex, symbol, timeframe: str, lookback: str):
...
def train_model_bundle(ex, symbol: str, timeframe: str, lookback: str):
    """Fetch data, build features, train a small classifier. Return bundle or None."""
    try:
        df_train = fetch_ohlcv_lookback_safe(ex, symbol, timeframe=timeframe, lookback=lookback)
        ...
        # --- Guard: must have at least 2 classes ---
        try:
        ...
        # --- Guard: effective rows after feature/shift/drop ---
        ...
        clf = GradientBoostingClassifier(random_state=42)
        clf.fit(X[cols], y)
        # Remove sklearn's feature-name metadata so inference with numpy arrays won't warn
        try:
            if hasattr(clf, "feature_names_in_"):
                delattr(clf, "feature_names_in_")
        except Exception:
...
def ensure_models_for_symbol(ex, symbol: str, timeframe: str):
    ...
    if not _os.path.isfile(p_main):
        ...
        b_main = train_model_bundle(ex, symbol, timeframe, main_lb)
        # main
    ...
    if not _os.path.isfile(p_fb):
        ...
        b_fb = train_model_bundle(ex, symbol, timeframe, fb_lb)
        # fallback
        if b_fb is not None:
...
def print_potential_menu(pots):
    """Tampilkan 1 baris per kandidat (rapi, tanpa spam newlines)."""
    say(f"{BOLD}Top Potentials (fallback){RESET}")
    for i, r in enumerate(pots, start=1):
        ...
        say(f"{i:>2}) {r['symbol']} {r['side']} · p_dir≈{(p_dir or 0):.2f} "
...
# def list_top_any_candidates(ex, timeframe=None, lookback="60d", universe_topn=120, topk=10, min_r=1.2):
#     """Very lenient: Top-N by volume → hitung potensi sederhana supaya SELALU ada menu.
#     Arah via EMA+fib_trend, coba Monte Carlo, kalau gagal pakai grid ATR.
#     Bentuk dict kompatibel dengan print_potential_menu/prompt_pick_potential.
#     """
#     timeframe = require_tf()
#     out = []
#     try:
#         rows = fetch_top_symbols(ex, limit=max(10, int(universe_topn)))
#         syms = [s for (s,_,_) in rows]
#         for s in syms:
#             try:
#                 df = fetch_ohlcv_lookback(ex, s, timeframe=timeframe, lookback=lookback)
#                 if len(df) < 120:
#                     continue
#                 Xf, cols = build_features(df)
#                 if len(Xf) < 120:
#                     continue
#                 ef = float(Xf["ema_fast"].iloc[-1]); es = float(Xf["ema_slow"].iloc[-1])
#                 ft = int(Xf["fib_trend"].iloc[-1]) if "fib_trend" in Xf.columns else 0
...
#                 # Monte Carlo; kalau None → fallback ATR grid dengan min_r longgar
#                 best = select_tp_sl_by_potential(Xf["close"], A, price, side, min_r=float(min_r))
#                 if best is None:
...
#             except Exception:
#                 continue
#         out.sort(key=lambda x: (x.get("exp") if x.get("exp") is not None else 0.0,
#                                 x.get("p_dir",0.0), -x.get("prox",9.99)), reverse=True)
#         return out[:max(1, int(topk))]
#     except Exception:
#         return []
...
def market_specs(ex, symbol):
    ...
    return int(amount_prec), int(price_prec), float(min_qty), float(min_cost)
    ...
    try:
        ...
        if max_lev:
            try:
                ...
                if max_lev >= 2:
                    return max(1.0, min(10.0, max_lev/2.0)), "market_limits"
            except Exception:
                pass
    except Exception:
        pass
...
def micro_tf_confirm(ex, symbol, tf="5m"):
    ...
    """
    try:
        dfm = cached_fetch_ohlcv(ex, symbol, timeframe=tf, candles=400)
...
def scalping_filter(ex, symbol, tf_main, df_main, side_intent, ctx_model):
    ...
    # Trend filters: ADX & BBW → attach to ctx
    try:
...
def _hard_entry_gates(side_intent, ctx, tf_main):
    blocks = []
    ok = True
    # … lalu semua gate hanya: if ...: ok=False; blocks.append("…")
    return ok, blocks
    """Stricter must-pass gates to avoid low-quality entries that often hit SL.
    Returns (ok: bool, blocks: list[str]). Controlled by ENV flags:
      AUTOPILOT_REQUIRE_VWAP (default 1)
      AUTOPILOT_REQUIRE_MICRO (default 1)
      AUTOPILOT_REQUIRE_FVG_ALIGN (default 1)
      AUTOPILOT_REQUIRE_SRBRPB (default 1)  # require any of SR support/resistance, BreakRetest, or Pullback OK
      AUTOPILOT_FVG_PROX_GATE (default 0.004)
      AUTOPILOT_VWAP_BAND_MULT_1M (default 2.0)  # looser band for minute TFs
    """
    import os, math

    # collector & base state
    blocks: list[str] = []
    ok = True

    # Required components (ENV-controlled)
    ...
    # Context values (robust to None/NaN)
    adx_v = ctx.get("adx")
    bbw_v = ctx.get("bbw")
    try:
        adx_v = float(adx_v)
    ...
    if REQ_ADX and (adx_v is None or (isinstance(adx_v, float) and math.isnan(adx_v)) or adx_v < ADX_MIN):
        ok = False; blocks.append(f"adx<{ADX_MIN:.0f}")
    if REQ_BBW and (bbw_v is None or (isinstance(bbw_v, float) and math.isnan(bbw_v)) or bbw_v < BBW_MIN):
        ok = False; blocks.append(f"bbw<{BBW_MIN:.3f}")

    # VWAP band (use SC knob; widen a bit on minute TF to account for noise)
    try:
        vwap_band = float(os.getenv("AUTOPILOT_SC_VWAP_BAND_PCT", "0.0018"))
    except Exception:
        vwap_band = 0.0018
    ...
    # Gate 1: VWAP proximity
    if require_vwap and (vdist is None or (isinstance(vdist, (int, float)) and vdist > vwap_thr)):
        ok = False; blocks.append("vwap")

    # Gate 2: Micro-trend alignment
    if require_micro:
        if not ((side_intent == "long" and micro == "up") or (side_intent == "short" and micro == "down")):
            ok = False; blocks.append("micro")

    # Gate 3: FVG alignment + reasonable proximity (in-gap or near)
    if require_fvg:
    ...
    # Gate 4: Any of SR confluence / Breakout-Retest / Pullback OK
    if require_any:
...
def wait_for_entry(ex, symbol, tf_main, side_intent, min_score=0.60, poll_sec=1.0, max_wait_sec=None):
    ...
    tf_main = require_tf()
    # Ensure both main & fallback models exist for this symbol+TF
    try:
        ensure_models_for_symbol(ex, symbol, tf_main)
    except Exception as _e_train:
        say(f"{YELLOW}TRAIN warn: {_e_train}{RESET}")
    _print_inline(f"ENTRY-WAIT {symbol} {side_intent} | threshold={min_score:.2f} | press 'c'+Enter to cancel")
    t0 = time.time()
    while True:
        score = 0.0
        try:
            df_main = fetch_ohlcv(ex, symbol, timeframe=tf_main, candles=400)
            score, reasons, ctx = scalping_filter(ex, symbol, tf_main, df_main, side_intent, {})
            msg = f"ENTRY {symbol} {side_intent} score={score:.2f} | " + ("OK" if score>=min_score else "wait")
            if reasons:
                msg += " | " + ",".join(reasons[:3])
            _print_inline(msg)
        except Exception as e:
        ...
        # cancel?
        s = _readline_nowait()
        if s and s.strip().lower() == "c":
            print()  # turun 1 baris sekali saja agar rapi
            header("Position Rationale")
            return False

        if score >= float(min_score):
            print()  # kunci baris OK
            return True

        if (max_wait_sec is not None) and (time.time() - t0 > max_wait_sec):
            print()
            say(f"{YELLOW}Entry-wait timeout.{RESET}")
            return False
...
def fetch_ohlcv(ex, symbol, timeframe="1m", candles=8000):
    tf = normalize_tf(timeframe)
    left=candles; since=None; rows=[]; per=min(1000, candles)
    while left>0:
...
def fetch_ohlcv_lookback(ex, symbol, timeframe="1m", lookback="1y"):
    """
    Fetch OHLCV from `lookback` ago (e.g., '1y','180d','26w','6m') up to now using CCXT pagination.
    ...
    df = to_df(rows)
    # dedupe/clip to exact lookback window
    start_dt = pd.Timestamp.now(tz=timezone.utc) - pd.Timedelta(days=days+1)
    df = df[df["datetime"] >= start_dt.tz_localize(None)].reset_index(drop=True)
    return df
...
def compute_size_from_risk(balance_free_usdt, entry, sl, amount_prec, min_qty, min_cost, leverage=1.0):
    ...
    # pembulatan akhir & validasi minQty
    qty = float(round(qty, int(amount_prec)))
    if qty < min_qty:
        return 0.0
    return qty
...
def signal_long_short(df_feat, feature_cols, bundle):
    ...
    prob = model.predict_proba(df_feat[cols])
...
#     TF = os.getenv("AUTOPILOT_TIMEFRAME", "1h").strip()
...
#     MAX_SPREAD = float(os.getenv("AUTOPILOT_MAX_SPREAD", "0.0015"))
#     PICK_RANK = int(os.getenv("AUTOPILOT_PICK_RANK", "1"))
#     TP_ATR = float(os.getenv("AUTOPILOT_TP_ATR", "1.8"))
#     SL_ATR = float(os.getenv("AUTOPILOT_SL_ATR", "1.0"))
#     MIN_R  = float(os.getenv("AUTOPILOT_MIN_R", "1.5"))
#     TRAIL_ON   = str(os.getenv("AUTOPILOT_TRAIL_ON", "1")).lower() not in ("0","false","no","off")
#     TRAIL_ATR  = float(os.getenv("AUTOPILOT_TRAIL_ATR", "0.8"))
#     DRYRUN = str(os.getenv("AUTOPILOT_DRYRUN", "1")).lower() not in ("0","false","no","off")
...
#     ranked = screen_usdtm_symbols(
#         ex, timeframe=TF, lookback=LOOKBACK,
#         min_edge=MIN_EDGE, min_pdir=MIN_PDIR,
#         fib_eps=FIB_EPS, max_spread=MAX_SPREAD,
#     )
#     if not ranked:
#         # say(f"{YELLOW}Tidak ada simbol lolos screening.{RESET}")
#         # return
#         say("Tidak ada simbol lolos screening.")
#     # Fallback: tampilkan 10 kandidat potensial → pilih → wait signal → trade/monitor
#     try:
#         SC_MIN = float(os.getenv("AUTOPILOT_SC_MIN_SCORE", "0.60"))
#         COOL   = int(float(os.getenv("AUTOPILOT_COOLDOWN_SEC", "60")))
#         TIGHTEN_BE = str(os.getenv("AUTOPILOT_TIGHTEN_BREAKEVEN", "1")).lower() not in ("0","false","no","off")
#         TIGHTEN_AT_R = float(os.getenv("AUTOPILOT_TIGHTEN_AT_R", "0.5"))
#         FALLBACK_UNIV = int(float(os.getenv("AUTOPILOT_FALLBACK_TOPN", "60")))
#     except Exception:
#         SC_MIN = 0.60; COOL = 60; TIGHTEN_BE = True; TIGHTEN_AT_R = 0.5; FALLBACK_UNIV = 60

#     while True:
#         pots = list_top_potentials_fallback(ex, timeframe=TF, lookback=LOOKBACK, universe_topn=FALLBACK_UNIV, topk=10, min_r=MIN_R)
#         if not pots:
#             say(f"{YELLOW}Tidak ada kandidat potensial (fallback).{RESET}")
#             return
#         print_potential_menu(pots)
#         pick = prompt_pick_potential(pots)
#         if not pick:
#             say(f"{YELLOW}Batal pilih kandidat.{RESET}")
#             return

#         symbol = pick["symbol"]; side = pick["side"]
#         price = float(pick.get("price") or 0.0)
#         A = float(pick.get("A") or 0.0)
#         ctx = pick.get("ctx") or {}

#         # Jelaskan konteks posisi
#         try:
...
#             sup = res = None
#             if sr.get("bias") == "support":
#                 sup = px_now * (1 - (sr.get("dist") or 0.0))
#             elif sr.get("bias") == "resistance":
#                 res = px_now * (1 + (sr.get("dist") or 0.0))
#             explain_entry(symbol, side, px_now, sup, res, A or 0.0, ctx, pick.get("sl") or px_now, pick.get("tp") or px_now, df_last.tail(120))
#         except Exception:
#             pass

#         # Waiting signal 1 detik; bisa cancel 'c'+Enter → balik ke daftar
#         ok = wait_for_entry(ex, symbol, TF, side, min_score=SC_MIN, poll_sec=1.0, max_wait_sec=None)
#         if not ok:
#             continue  # kembali ke menu kandidat

#         # Harga/ATR terbaru sebelum risk sizing
#         tkr = ex.fetch_ticker(symbol)
#         price_live = float(tkr.get("last") or tkr.get("close") or tkr.get("bid") or 0.0) or price
#         try:
...
#         # Auto TP/SL berbasis potensi (tanpa trailing TP; SL boleh dinaikkan nanti)
#         best = select_tp_sl_by_potential(df_live["close"] if 'df_live' in locals() and not df_live.empty else pd.Series([price_live]),
...
#             qty = float(qty)
#         if qty <= 0:
#             say(f"{YELLOW}Qty<=0 setelah sizing. Batal trade.{RESET}")
#             return

#         # DRYRUN?
#         DRY = str(os.getenv("AUTOPILOT_DRYRUN", "1")).lower() not in ("0","false","no","off")
#         if DRY:
#             say(f"{CYAN}DRYRUN aktif — tidak kirim order.{RESET}")
...
#     k = max(1, min(PICK_RANK, len(ranked))) - 1
#     pick = ranked[k]
#     symbol = pick["symbol"]
#     say(f"Memilih #{k+1}: {symbol} | side={pick['side']} score={pick.get('score'):.3f} p_dir={pick['p_dir']:.2f} edge={pick['edge']:.2f}")

#     # Build features & model for the picked symbol
#     header("Build features + model")
#     df = fetch_ohlcv_lookback(ex, symbol, timeframe=TF, lookback=LOOKBACK)
#     Xf, cols = build_features(df)
#     Xf["ATR"] = atr(Xf, 14)
#     bundle, _ = train_or_load_model(symbol, TF, Xf, cols)
#     side, conf, ctx = signal_long_short(Xf, cols, bundle)
#     if side is None:
#         say(f"{YELLOW}Model tidak menemukan bias yang jelas saat ini. Batal.{RESET}")
#         return

#     price = float(Xf["close"].iloc[-1])
#     A = float(Xf["ATR"].iloc[-1])
#     swings_lo, swings_hi = structure_levels(Xf)
#     sup, res = nearest_levels(price, swings_lo, swings_hi)

#     # === Risk params (dynamic potentials) ===
#     best = select_tp_sl_by_potential(Xf["close"], A, price, side, min_r=MIN_R)
#     if best:
#         sl = best["sl"]; tp = best["tp"]
#         say(f"Potensi: P(TP)≈{best['p']:.2f} | R≈{best['r']:.2f} | ExpR≈{best['exp']:.2f} | TPx={best['tp_m']:.2f} SLx={best['sl_m']:.2f}")
#     else:
...
#     # Tighten SL ke struktur (tanpa trailing TP)
#     if side == "long" and sup is not None:
#         sl = max(sl, min(sup, price))
#     if side == "short" and res is not None:
#         sl = min(sl, max(res, price))

#     # === ENTRY-WAIT (bisa cancel dengan 'c'+Enter) ===
#     if ENTRY_WAIT_ON:
#         ok = wait_for_entry(ex, symbol, TF, side, min_score=SC_MIN, poll_sec=1.0, max_wait_sec=None)
#         if not ok:
#             # kembali ke Position Rationale
#             explain_entry(symbol, side, price, sup, res, A, ctx, sl, tp, Xf.tail(120))
#             say(f"{YELLOW}Entry dibatalkan oleh user.{RESET}")
#             return

#     # Tanpa trailing TP; boleh naikkan SL ke struktur jika lebih ketat (risk ↓)
#     if side == "long" and sup is not None:
#         sl = max(sl, min(sup, price))
#     if side == "short" and res is not None:
#         sl = min(sl, max(res, price))
...
#     # Leverage/mode
#     lev, mode, meta = decide_leverage_mode(
#         ctx, A, price,
#         spread=pick.get('spread'),
#         funding=pick.get('funding'),
#     )

#     # Balance and market specs
#     total, free, used = get_futures_usdt_balance(ex)
#     amount_prec, price_prec, min_qty, min_cost = market_specs(ex, symbol)

#     # Position sizing
#     qty = compute_size_from_risk(free, price, sl, amount_prec, min_qty, min_cost, leverage=lev)
#     if qty <= 0:
#         qty_min, lev_used, changed, note = auto_scale_for_min_cost(ex, symbol, free, price, min_qty, min_cost)
#         if qty_min > 0:
#             qty = qty_min; lev = lev_used
#         else:
#             say(f"{YELLOW}Ukuran posisi 0 (saldo/leverage tak cukup). Batal. Note: {note}{RESET}")
#             return

#     # Explain plan
#     explain_entry(symbol, side, price, sup, res, A, ctx, sl, tp, Xf.tail(120))
#     say(f"Leverage: target={lev}x (mode={mode}) | free USDT≈{free:.2f}")
#     say(f"Qty≈{qty} @ ~{price:.{price_prec}f} | SL={sl:.{price_prec}f} | TP={tp:.{price_prec}f}")
#     if TRAIL_ON:
#         say(f"Trailing aktif: {TRAIL_ATR} ATR")

#     # Execution (dry-run default)
#     DRY = DRYRUN
#     if DRY:
#         say(f"{CYAN}DRYRUN aktif — tidak mengirim order nyata.{RESET}")
#         log_signal_txt(symbol, side, price, tp, sl, leverage=lev, margin_mode=mode, dryrun=True)
#         return

#     # Try set leverage/mode best-effort; ignore failures
#     try:
#         set_symbol_leverage(ex, symbol, lev, os.getenv("AUTOPILOT_MARGIN_MODE", "crossed"))
#     except Exception:
#         pass
...
#     except Exception as e:
#         monitor_until_exit(ex, symbol, side, price, sl, tp, qty, price_prec,
#                            tighten_be_on=TIGHTEN_BE, tighten_at_R=TIGHTEN_AT_R, cooldown_sec=COOL)
#         say(f"{RED}Gagal kirim order: {e}{RESET}")
#         return

#     try:
#         log_signal_txt(symbol, side, price, tp, sl, leverage=lev, margin_mode=mode, dryrun=False)
#     except Exception:
#         pass
...
def autopilot():
    ...
    # Config
    TF = os.getenv("AUTOPILOT_TIMEFRAME", "1h")
    LOOKBACK = os.getenv("AUTOPILOT_TRAIN_LOOKBACK", "180d")
    ...
    try:
        while True:
            ...
            # Bangun daftar Top 10 selalu tersedia
            pots = []
            if cands:
                cand_syms = [r.get("symbol") for r in cands if r.get("symbol")]
                cand_syms = cand_syms[:max(15, MENU_TOPK*2)]
                pots = list_potentials_for_symbols(
                    ex, cand_syms, timeframe=TF,
                    lookback=os.getenv("AUTOPILOT_FALLBACK_LOOKBACK", LOOKBACK),
                    topk=MENU_TOPK, min_r=MIN_R
                )
            if not pots:
                pots = list_top_potentials_fallback(
                    ex, timeframe=TF,
                    lookback=os.getenv("AUTOPILOT_FALLBACK_LOOKBACK", LOOKBACK),
                    universe_topn=int(float(os.getenv("AUTOPILOT_FALLBACK_TOPN", "80"))),
                    topk=MENU_TOPK, min_r=MIN_R
                )

            if not pots:
                # Bangun Top-10 yang sangat lenient supaya kamu bisa pilih TICKER SEKARANG
                pots = list_top_any_candidates(
                    ex,
                    timeframe=TF,
                    lookback=os.getenv("AUTOPILOT_FALLBACK_LOOKBACK", LOOKBACK) or LOOKBACK,
                    universe_topn=int(float(os.getenv("AUTOPILOT_FALLBACK_TOPN", "120"))),
                    topk=MENU_TOPK,
                    min_r=max(1.0, MIN_R - 0.5),
                )
                if not pots:
                    # Last resort: menu sederhana dari top-volume (EMA saja, tanpa MC)
                    try:
                        rows = fetch_top_symbols(ex, limit=max(20, MENU_TOPK*3))
                    except Exception:
                        rows = []
                    simple = []
                    for (s, last, vol) in rows:
                        try:
                            df_ = fetch_ohlcv_lookback(ex, s, timeframe=TF, lookback="30d")
                            if len(df_) < 60:
                                continue
                            df_["ema_fast"] = ema(df_["close"], 20)
                            df_["ema_slow"] = ema(df_["close"], 50)
                            ef = float(df_["ema_fast"].iloc[-1]); es = float(df_["ema_slow"].iloc[-1])
                            side = "long" if ef >= es else "short"
                            A_ = float(atr(df_, 14).iloc[-1]); price_ = float(df_["close"].iloc[-1])
                            simple.append({
                                "symbol": s, "side": side, "p_dir": 0.55,
                                "edge": 0.10 if side=="long" else -0.10, "prox": 0.0100,
                                "atr_pct": float((A_/price_) if price_ else 0.0),
                                "price": float(price_), "A": float(A_),
                                "p_mc": None, "exp": 0.0, "tp_m": None, "sl_m": None, "tp": None, "sl": None,
                                "ctx": {"pL":0.55 if side=='long' else 0.45, "pS":0.45 if side=='long' else 0.55,
                                        "edge":0.10 if side=='long' else -0.10, "ema_fast":ef, "ema_slow":es,
                                        "fib_trend": 1 if side=='long' else -1, "fib_dist_500":0.01, "fib_dist_618":0.01},
                            })
                        except Exception:
                            continue
                    pots = simple[:MENU_TOPK]
                if not pots:
                    say(f"{YELLOW}Masih tidak ada kandidat; ulang screening {IDLE_SLEEP}s…{RESET}")
            ...
            pick = prompt_pick_potential(pots)
            if not pick:
                say(f"{YELLOW}Tidak memilih; ulang screening…{RESET}")
                time.sleep(1)
                continue
